...
The purpose is to get more familiar with working with the Hector simple cliamte model. 
...
Then, I retrieved results from the carbon pools. 
...
In order to have a more precise method besides brute forcing it, I modified and
added to the sensitivity analysis Vignette in the Hector Manual. 
...
This function uses the previous function to run a range of values for a specified
parameter and maps them to each other 
```{r paramRange}
...
the cutoff for the closest the varaible can be to the original value while still
...
by myself. I created this function to repeat this process several times. 
```{r sensitivity}
...
It choses the q10_rh closest to the original q10_rh value, by utilizing the above 
...
In this case it is NA, because no value of q10_rh would create that low of a flux when
