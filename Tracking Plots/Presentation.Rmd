...
getResults <- function(ini_file, scenarioName){
  ...
  result_vars <- c(FFI_EMISSIONS(), DACCS_UPTAKE(),ATMOSPHERIC_CO2(), GLOBAL_TEMP())
...
scenarioGraph <- ggplot(hectorResults)+
  ...
  facet_wrap(~variable, scales = "free_y", 
             labeller = labeller(variable = c("ffi_emissions" = "Fossil Fuel Emissions",
                                              "daccs_uptake" = "DACCS Uptake",
                                              "Ca" = "CO2 Concentration (ppmv CO2)",
...
```{r combinedFracionPLot, fig.width=9, fig.height=7}
atmos_results_all <-
  atmos_results_all %>%
  mutate(year=year+1)
...
graph <- ggplot(df) +
  ...
  ylab("Airborne Fraction in 2000")+
  xlab(NULL)
